...
public class ShootAbility : Ability
{
    ...
    public override void Activate(GameObject instigator, GameObject target = null)
    {
        ...
        GameObject weaponSocket = character.CharacterView.m_weaponSocket.gameObject;
        Animator animator = weaponSocket.GetComponent<Animator>();
        animator.Play("Character_Weapon_Idle", 0, 0f);
        animator.Update(0f);
        animator.SetTrigger("Attack");

        GameObject weaponEndSocket = character.CharacterView.m_weaponEndSocket.gameObject;
        var temp = weaponEndSocket.transform.Find("PS_Dust");
        ParticleSystem ps = temp.gameObject.GetComponent<ParticleSystem>();
        if (ps)
        {
            ps.Play();
        }
